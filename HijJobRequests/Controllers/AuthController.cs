...
namespace HijJobRequests.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        [HttpPost("login")]
        public async Task<ActionResult<string>> VerifyLogin(string Identityfier, string Password, string Company)
        {
            return Ok((await _context.AppUsers.FirstOrDefaultAsync(x => x.FEmail.Equals(Identityfier) || x.FJawNo.Equals(Identityfier) || x.FIdNo.Equals(Identityfier) && x.FUserPass.Equals(Password))) is not null ? SetAppUserOtp(Identityfier) : string.Empty);
        }

        [HttpPost("VerifyOtp")]
        public async Task<ActionResult<string>> VerifyOtp(string Identityfier, string Otp, string Company)
        {
            SharedVars.AppUsersOtps.RemoveWhere(x => x.Item3 < DateTime.UtcNow);
            return Ok(SharedVars.AppUsersOtps.Any(x => x.Item1.Equals(Identityfier) && x.Item2.Equals(Otp)) ? await GenerateToken(Identityfier, Company) : string.Empty);
        }
