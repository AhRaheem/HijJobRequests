using System;
using System.Collections.Generic;

namespace HijJobRequests.Models;

public partial class TdTransport
{
    public DateTime FLastUpdate { get; set; }

    public decimal FLastUpdateUser { get; set; }

    public decimal FLastUpdateSum { get; set; }

    public decimal FLastUpdateOper { get; set; }

    public decimal FLastUpdateOperNo { get; set; }

    public decimal FTransportNo { get; set; }

    public string FTransportName { get; set; } = null!;

    public decimal FTransportMin { get; set; }

    public decimal FTransportStatus { get; set; }
}
